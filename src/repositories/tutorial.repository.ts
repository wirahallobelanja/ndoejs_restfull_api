...
class TutorialRepository implements ITutorialRepository {
  save(tutorial: Tutorial): Promise<Tutorial> {
    return new Promise((resolve, reject) => {
      console.log("this is tutorial", tutorial);
      
      connection.query<OkPacket>(
        "INSERT INTO tutorials (title, description, published, url_link) VALUES(?,?,?,?)",
        [tutorial.title, tutorial.description, tutorial.published ? tutorial.published : false, tutorial.url_link],
        (err, res) => {          
          if (err) reject(err);
          else
  ...
  delete(tutorialId: number): Promise<number> {
    return new Promise((resolve, reject) => {
      console.log("this is id", tutorialId);
  ...
  deleteAll(): Promise<number> {
    return new Promise((resolve, reject) => {
      connection.query<OkPacket>("DELETE FROM tutorials WHERE id = ?", (err, res) => {
        if (err) reject(err);
        else resolve(res.affectedRows);
      });
    });
